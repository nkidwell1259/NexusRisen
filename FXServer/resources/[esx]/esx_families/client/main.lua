...
function OpenCloakroomMenu()
  ...
  local elements = {
    ...
    {label = _U('grove_wear'), value = 'grove_wear'}
  }
  ...
  if Config.EnableNonFreemodePeds then
    ...
    ESX.UI.Menu.Open(
      ...
      if data.current.value == 'grove_wear' then
  ...
  if Config.EnableArmoryManagement then
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        TriggerServerEvent('esx_grovejob:giveWeapon', weapon,  1000)
      end,
      function(data, menu)
...
function OpenGroveActionsMenu()
  ...
  ESX.UI.Menu.Open(
    'default', GetCurrentResourceName(), 'grove_actions',
    {
      title    = 'Grove',
      align    = 'bottom-right',
      elements = {
          ...
          function(data2, menu2)
            ...
            if distance ~= -1 and distance <= 3.0 then
              ...
              if data2.current.value == 'handcuff' then
                TriggerServerEvent('esx_grovejob:handcuff', GetPlayerServerId(player))
              end

              if data2.current.value == 'drag' then
                TriggerServerEvent('esx_grovejob:drag', GetPlayerServerId(player))
              end

              if data2.current.value == 'put_in_vehicle' then
                TriggerServerEvent('esx_grovejob:putInVehicle', GetPlayerServerId(player))
              end

              if data2.current.value == 'out_the_vehicle' then
                  TriggerServerEvent('esx_grovejob:OutVehicle', GetPlayerServerId(player))
              end
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        if data.current.value ~= nil then
          ...
          TriggerServerEvent('esx_grovejob:confiscatePlayerItem', GetPlayerServerId(player), itemType, itemName, amount)
...
function OpenFineCategoryMenu(player, category)
  ...
  ESX.TriggerServerCallback('esx_grovejob:getFineList', function(fines)
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        if Config.EnablePlayerManagement then
          TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(player), 'society_grove', _U('fine_total') .. label, amount)
        else
...
function OpenVehicleInfosMenu(vehicleData)
  ...
  ESX.TriggerServerCallback('esx_grovejob:getVehicleInfos', function(infos)
...
AddEventHandler('esx_grovejob:hasEnteredMarker', function(station, part, partNum)
  ...
  if part == 'HelicopterSpawner' then
    ...
    local helicopters = Config.GroveStations[station].Helicopters
...
AddEventHandler('esx_grovejob:hasEnteredEntityZone', function(entity)
  ...
  if PlayerData.job ~= nil and PlayerData.job.name == 'grove' and not IsPedInAnyVehicle(playerPed, false) then
...
AddEventHandler('esx_grovejob:drag', function(cop)
...
AddEventHandler('esx_grovejob:OutVehicle', function(t)
...
Citizen.CreateThread(function()
  while true do
    ...
    if CurrentAction ~= nil then
      ...
      if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'grove' and (GetGameTimer() - GUI.Time) > 150 then
   ...
   if IsControlPressed(0,  Keys['F6']) and PlayerData.job ~= nil and PlayerData.job.name == 'grove' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'grove_actions') and (GetGameTimer() - GUI.Time) > 150 then
...
RegisterNetEvent('NB:openMenuGrove')
AddEventHandler('NB:openMenuGrove', function()
	OpenGroveActionsMenu()
end)
